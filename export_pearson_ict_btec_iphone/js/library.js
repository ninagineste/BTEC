...
*/(function($) {
  ...
  handler = function(event) {
    var args, delta, deltaX, deltaY, orgEvent, returnValue;
  ...
  $.event.special.mousewheel = {
    setup: function() {
      ...
      if (this.addEventListener) {
        ...
        _results = [];
        while (i) {
          _results.push(this.addEventListener(types[--i], handler, false));
        }
        return _results;
      } else {
    ...
    teardown: function() {
      ...
      if (this.removeEventListener) {
        ...
        _results = [];
        while (i) {
          _results.push(this.removeEventListener(types[--i], handler, false));
        }
        return _results;
      } else {
...
(function(window, document, $, undefined_) {
        ...
        if ($.type(element) === "object") {
          ...
          if (isQuery(element)) {
            obj = {
              ...
              copyright: element.attr("data-copyright"),
  ...
  F.helpers.title = {
    beforeShow: function(opts) {
      var copyright, counter, num, number, text, title, type;
      text = F.current.title;
      num = F.current.num;
      copyright = F.current.copyright;
      number = F.current.number;
      type = opts.type;
      title = void 0;
      if (number) {
        counter = "<div class='counter'>" + number + "</div>";
      } else {
        counter = "none";
      }
      if (copyright) {
        title = $("<div class=\"fancybox-title fancybox-title-" + type + "-wrap\">" + "<span class=\"identifier\">" + num + "</span> " + text + "</div><div class=\"copyright\">" + copyright + "</div>" + counter).appendTo("body");
      } else {
